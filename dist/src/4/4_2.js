...
function isBoardComplete(board, drawnNumbers) {
    const matchingRows = board.rows.some((row) => areAllElementsInDrawnNumbers(row, drawnNumbers));
    const matchingCols = board.cols.some((col) => areAllElementsInDrawnNumbers(col, drawnNumbers));

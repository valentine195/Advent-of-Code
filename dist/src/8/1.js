...
const input = (0, helper_1.read)("src/8/input.txt");
...
const start = (input) => {
    ...
    toMatch.forEach((str) => {
        str?.forEach((letters) => {
            if (lengths.has(letters.length) &&
                lengths.get(letters.length).length == 1) {
                counts.set(lengths.get(letters.length), (counts.get(lengths.get(letters.length)) ?? 0) + 1);
            }
        });
    });
    console.log("🚀 ~ file: 1.ts ~ line 55 ~ counts", counts, (0, helper_1.sum)(counts));
};
//run(start, input, testInput);
const sort = (str = "") => {
    return str.split("").sort().join("");
};
const includes = (str, rem = "") => {
    const split = str.split("");
    return rem.split("").every((v) => split.includes(v));
};
const remove = (str, rem = "") => {
...
const start2 = (input) => {
    ...
    for (let [original, output] of data) {
        ...
        map.set(3, signal.find((v) => v.length == 5 && remove(v, map.get(7)).length == 2));
        signal = signal.filter((v) => ![...map.values()].includes(v));
        map.set(9, signal.find((v) => v.length == 6 && remove(v, map.get(3)).length == 1));
        signal = signal.filter((v) => ![...map.values()].includes(v));
        map.set(0, signal.find((v) => v.length == 6 &&
            includes(v, map.get(1)) &&
            remove(v, map.get(1)).length == 4));
        signal = signal.filter((v) => ![...map.values()].includes(v));
        map.set(6, signal.find((v) => v.length == 6 && remove(v, map.get(3)).length == 2));
        signal = signal.filter((v) => ![...map.values()].includes(v));
        map.set(5, signal.find((v) => v.length == 5 && remove(v, map.get(9)).length == 0));
        signal = signal.filter((v) => ![...map.values()].includes(v));
        map.set(2, signal.find((v) => v.length == 5));
        console.log("🚀 ~ file: 1.ts ~ line 93 ~ map", map);
        const inv = new Map([...map.entries()].map((m) => [m[1], m[0]]));
        const value = output.map((v) => inv.get(v));
        /*         console.log(
            "🚀 ~ file: 1.ts ~ line 158 ~ output",
            output,
            original.map((v) => sort(v))
        ); */
        values.push(Number(value.join("")));
    }
    console.log(values, (0, helper_1.sum)(values));
};
(0, helper_1.run)(start2, input, testInput);
